...
<?= $arResult['BLOCK_START'] ?>
    ...
    <div>
        <? if (!empty($arResult['ITEMS'])) : ?>
            ...
            <div class="block16-1-bg-image lazy" <?if($useLazyLoad && $arItem['BG_IMG']):?>data-bg="<?= $arItem['BG_IMG'] ?>"<?endif?>
                 style="<?if(!$useLazyLoad && $arItem['BG_IMG']):?>background-image: url(<?= $arItem['BG_IMG'] ?>);<?endif?><?if($arItem['PROPS']['BG_COLOR']):?>background-color: <?=$arItem['PROPS']['BG_COLOR']?><?endif?>">
                <div class="maxwidth-theme height-inherit">
                    <div class="row height-fill">
                        ...
                        <div class="block-title col-lg-<?= !empty($arItem['IMG']) ? '6' : '8' ?> block16-1-content-<?=!empty($arItem['IMG']) ? 'left' : 'center' ?> <?= $lightTextClasses ?>">
                            ...
                            <? if (!empty($arItem['PRICE']) || !empty($arItem['OLD_PRICE'])) : ?>
                                <div class="block16-1-prices  <? if (empty($arItem['IMG'])) : ?> block16-1-prices-center <? endif ?>">
                                    <? if (!empty($arItem['PRICE'])) : ?>
                                        <div class="block16-1-price"><?= Helper::money($arItem['PRICE']) ?></div>
                                    <? endif ?>
                                    <? if (!empty($arItem['OLD_PRICE'])) : ?>
                                        <div class="block16-1-price-old"><?= Helper::money($arItem['OLD_PRICE']) ?></div>
                                    <? endif ?>
                                </div>
                            <? endif ?>
                        ...
                        <? if (!empty($arItem['IMG']) || !empty($arItem['BG_IMG']) ): ?>
                            <div class="col-lg-6 col-flex-rw p-0 lazy block16-1-image <? if (empty($arItem['IMG'])): ?> block16-1-no-image <? endif ?>"
