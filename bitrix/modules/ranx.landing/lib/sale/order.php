...
class Order
{
    ...
    private static function prepareData($data)
    {
        ...
        if (!empty($data['IS_ONECLICK'])) {
            $product = Basket::fetchItemData($data['PRODUCT_ID'] ?? 0);
    ...
    private static function createIblock($data)
    {
        ...
        if (!($data = self::prepareData($data))) {
            throw new Exception('Order prepareData failed');
        }
        ...
        if ($id <= 0) {
            throw new Exception(trim(strip_tags($el->LAST_ERROR)) ?: 'Order create failed');
        }
        ...
        // send() пишет в b_event; CheckEvents сразу отправляет, без дубля
