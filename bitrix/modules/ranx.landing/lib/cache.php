 ...
 * 
 * @author Konstantin Chechetkin - ASPRO
 */
class Cache
{
            ...
            if($arGroupBy === array()){
                ...
                if($bGetDetailPageUrlsArray){
                    ...
                    foreach($arByIBlock as $IBLOCK_ID => $arIB){
                        ...
                        if($arSectionIDs){
                            $arSections = Cache::CIBlockSection_GetList(array("CACHE" => array("TAG" => Cache::GetIBlockCacheTag($IBLOCK_ID), "MULTI" => "N", "GROUP" => array("ID"))), array("ID" => $arSectionIDs), false, array("ID", "CODE", "EXTERNAL_ID", "IBLOCK_ID"));
                        }
                        foreach($arIB as $SECTION_ID => $arIDs){
                            if($arIDs){
                                $rsElements = \CIBlockElement::GetList(array(), array("ID" => $arIDs), false, false, array("ID", "DETAIL_PAGE_URL", "CANONICAL_PAGE_URL"));
                                $rsElements->SetUrlTemplates(Cache::$arIBlocksInfo[$IBLOCK_ID]["DETAIL_PAGE_URL"]);
    ...
    public static function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
        ...
        if(!strlen($cachePath)){
            $cachePath = "/Cache/sale/CSaleBasket_GetList/".$cacheTag."/";
        }
        $obCache = new \CPHPCache();
    ...
    public static function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
        ...
        if(!strlen($cachePath)){
            $cachePath = "/Cache/forum/CForumMessage_GetListEx/".$cacheTag."/";
        }
        $obCache = new \CPHPCache();
    ...
    private static function _InitCacheParams($moduleName, $functionName, $arCache){
        ...
        if(!strlen($cachePath)){
            $cachePath = "/Cache/".$moduleName."/".$functionName."/".$cacheTag."/";
        }
    ...
    public static function GetIBlockCacheTag($IBLOCK_ID){
        if(!$IBLOCK_ID){
            ...
            return @Cache::$arIBlocksInfo[$IBLOCK_ID]["CODE"].$IBLOCK_ID;
        }
    }

    public static function ClearTagIBlock(){
        global $CACHE_MANAGER;
    ...
    public static function ClearTagIBlockElement($arFields){
        ...
        if($arFields["IBLOCK_ID"]){
            $CACHE_MANAGER->ClearByTag(Cache::GetIBlockCacheTag($arFields["IBLOCK_ID"]));
        }
    }
    ...
    public static function ClearTagIBlockSectionBeforeDelete($ID){
        global $CACHE_MANAGER;
        if($ID > 0){
            if($IBLOCK_ID = Cache::CIBlockSection_GetList(array("CACHE" => array("MULTI" => "N")), array("ID" => $ID), false, array("IBLOCK_ID"), true)){
                $CACHE_MANAGER->ClearByTag(Cache::GetIBlockCacheTag($IBLOCK_ID));
            }
        }
    }
}
