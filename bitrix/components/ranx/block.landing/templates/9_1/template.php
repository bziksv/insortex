...
<?= $arResult['BLOCK_START'] ?>
    ...
    <div class="maxwidth-theme">
        ...
        <div class="employees-wrapper slider--hide-counter columns--<?=$arResult['COLS']?> <?= $arResult['SLIDER'] ? 'employees--slider' : '' ?>">
            <?if ($arResult['SLIDER']):?>
            ...
            <div class="row employees">
                <?foreach($arResult['ITEMS'] as $arItem):?>
                    ...
                    <div class="col-lg-<?= $arResult['COLS'] ? 12 / $arResult['COLS'] : 3 ?> col-sm-6">

                        <div class="employee lazy"
                             <?if($arItem['IMG']):?>
                                 <?if($useLazyLoad):?> data-bg="<?=$arItem['IMG']?>"
                                 <?else:?> style="background-image: url(<?=$arItem['IMG']?>);"
                                 <?endif?>
                             <?endif?>>
                            <div class="employee-cover"></div>
...
<script>
    $(document).ready(function () {
        window['block_<?=$arResult['ID']?>_slider'] = new Block9_1Slider(<?=$arResult['ID']?>, <?=$arResult['COLS'] ?: 4?>);
    });
</script>
<?endif;?>
