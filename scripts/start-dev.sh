...
HTTP=$(curl -sS -o /tmp/insortex-check.html -w '%{http_code}' --max-time 60 http://localhost:8087/ || echo 000)

echo "http://localhost:8087/ → HTTP $HTTP (gentle: fpm ondemand max_children=3)"
